...
def verify_one(movie):
    try:
        if movie.get('type')!='movie' or 'comedy' not in movie.get('genres',[]): return (False,movie.get('titleEn'),'metadata')
        if not 90 <= int(movie.get('runtimeMinutes',0)) <= 210: return (False,movie.get('titleEn'),'runtime-metadata')
        sources=movie.get('sources') or []
        if not sources or not str(sources[0].get('url','')).startswith('https://'): return (False,movie.get('titleEn'),'source')
        url=sources[0]['url']
        ...
        if p.returncode: return (False,movie.get('titleEn'),'ffprobe')
        ...
        if not video or video[0] != 'h264': return (False,movie.get('titleEn'),f'video:{video[:1]}')
        if audio and audio[0] not in {'aac','mp3'}: return (False,movie.get('titleEn'),f'audio:{audio[:1]}')
        if not 5400 <= duration <= 12600: return (False,movie.get('titleEn'),f'duration:{duration}')
        ...
        if q.returncode: return (False,movie.get('titleEn'),'decode')
        return (True,movie.get('titleEn'),duration)
...
def main():
    ...
    report={'target':TARGET,'verified':TARGET,'distinct':TARGET,'generatedAt':dt.datetime.now(dt.timezone.utc).isoformat(),'method':'revalidated existing batch: ffprobe H.264/AAC-or-MP3 duration gate + real ffmpeg decode sample for all 100'}
