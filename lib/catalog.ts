...
export type Movie = { id:string; type:"movie"; genres:Genre[]; titleAr:string; titleEn:string; year:number; languageAr:string; languageEn:string; runtimeMinutes:number; poster:string; descriptionAr:string; descriptionEn:string; publishedAt:string; sources:VideoSource[]; subtitles:Subtitle[]; sourceUrl:string; licenseName:string; licenseUrl:string; attribution:string; downloadAllowed:boolean };
...
export const movieCatalog: Movie[] = [...baseMovies, ...(generatedComedy as Movie[])];
export const publicMovies = movieCatalog.filter(movie => movie.runtimeMinutes >= 90 && movie.sources.length > 0);
